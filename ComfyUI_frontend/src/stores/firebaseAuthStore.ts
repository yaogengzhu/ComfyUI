...
// When an external auth system (e.g. Electron app) provides a ComfyOrg token
// via localStorage, use it as a fallback when Firebase auth is not available.
const EXTERNAL_TOKEN_KEY = 'comfy_org_token'
const EXTERNAL_USER_KEY = 'huizhi_user_info'

function getExternalToken(): string | null {
  try {
    const token = localStorage.getItem(EXTERNAL_TOKEN_KEY)
...
export const useFirebaseAuthStore = defineStore('firebaseAuth', () => {
  ...
  // ============= Getters =============
  // Authenticated if Firebase user exists OR external token is present
  const isAuthenticated = computed(
    () => !!currentUser.value || !!getExternalToken()
  )

  const userEmail = computed(() => {
    if (currentUser.value?.email) return currentUser.value.email
    return getExternalUserInfo()?.email ?? null
  })

  const userId = computed(() => {
    if (currentUser.value?.uid) return currentUser.value.uid
    return getExternalUserInfo()?.uid ?? null
  })
  // ====================================
  ...
  // If external token exists, mark initialized immediately so the UI doesn't
  // wait for Firebase (which will never resolve a user for external tokens).
  if (getExternalToken()) {
  ...
  /**
   * Get ID token. Firebase first, then external token fallback.
   */
  const getIdToken = async (): Promise<string | undefined> => {
    ...
    // Fallback to external token
    return getExternalToken() ?? undefined
  }

  /**
   * Retrieves the appropriate authentication header for API requests.
   * Priority: workspace token > Firebase token > external token > API key.
   */
   ...
   * Falls back to external token if no Firebase user.
   */
   ...
   * Priority: workspace token > Firebase token > external token.
   */
