...
const Comfy_ConfirmClear = { "name": "ワークフローをクリアする際に確認を要求する" };
const Comfy_DOMClippingEnabled = { "name": "DOM要素のクリッピングを有効にする（有効にするとパフォーマンスが低下する可能性があります）" };
const Comfy_DevMode = { "name": "開発モードオプションを有効にする（API保存など）" };
...
const Comfy_DisableSliders = { "name": "ノードウィジェットスライダーを無効にする" };
const Comfy_EditAttention_Delta = { "name": "Ctrl+上/下の精度" };
const Comfy_EnableTooltips = { "name": "ツールチップを有効にする" };
const Comfy_EnableWorkflowViewRestore = { "name": "ワークフロー内のキャンバスの位置とズームレベルを保存および復元する" };
...
const Comfy_Graph_CanvasInfo = { "name": "左下隅にキャンバス情報を表示する（fpsなど）" };
const Comfy_Graph_CanvasMenu = { "name": "グラフキャンバスメニューを表示する" };
const Comfy_Graph_CtrlShiftZoom = { "name": "ファストズームショートカットを有効にする（Ctrl + Shift + ドラッグ）" };
...
const Comfy_Graph_ZoomSpeed = { "name": "キャンバスズーム速度" };
const Comfy_GroupSelectedNodes_Padding = { "name": "選択したノードのパディング" };
const Comfy_Group_DoubleClickTitleToEdit = { "name": "グループタイトルをダブルクリックして編集" };
...
const Comfy_NodeBadge_ShowApiPricing = { "name": "APIノードの料金バッジを表示" };
...
const Comfy_Node_AllowImageSizeDraw = { "name": "画像プレビューの下に幅×高さを表示する" };
...
const Comfy_Node_DoubleClickTitleToEdit = { "name": "ノードタイトルをダブルクリックして編集" };
const Comfy_Node_MiddleClickRerouteNode = { "name": "ミドルクリックで新しいリルートノードを作成" };
const Comfy_Node_Opacity = { "name": "ノードの不透明度" };
...
const Comfy_PromptFilename = { "name": "ワークフローを保存する際にファイル名を入力するよう促す" };
...
const Comfy_Sidebar_UnifiedWidth = { "name": "サイドバーの幅を統一" };
const Comfy_SnapToGrid_GridSize = {
	"name": "グリッドサイズにスナップ",
	"tooltip": "シフトを押しながらノードをドラッグおよびサイズ変更すると、グリッドに整列されます。これにより、そのグリッドのサイズが制御されます。"
};
const Comfy_TextareaWidget_FontSize = { "name": "テキストエリアウィジェットのフォントサイズ" };
const Comfy_TextareaWidget_Spellcheck = { "name": "テキストエリアウィジェットのスペルチェック" };
const Comfy_TreeExplorer_ItemPadding = { "name": "ツリーエクスプローラーアイテムのパディング" };
...
const Comfy_Validation_Workflows = { "name": "ワークフローを検証" };
...
const Comfy_Window_UnloadConfirmation = { "name": "ウィンドウを閉じるときに確認を表示" };
...
const Comfy_Workflow_ConfirmDelete = { "name": "ワークフローを削除する際に確認を表示" };
const Comfy_Workflow_Persist = { "name": "ページ（再）ロード時にワークフローの状態を保持し、復元します" };
const Comfy_Workflow_ShowMissingModelsWarning = { "name": "欠落しているモデルの警告を表示" };
const Comfy_Workflow_ShowMissingNodesWarning = { "name": "欠落しているノードの警告を表示" };
const Comfy_Workflow_SortNodeIdOnSave = { "name": "ワークフローを保存する際にノードIDをソート" };
const Comfy_Workflow_WarnBlueprintOverwrite = { "name": "既存のサブグラフブループリントを上書きする際に確認を要求" };
...
const LiteGraph_ContextMenu_Scaling = { "name": "ズームイン時にノードコンボウィジェットメニュー（リスト）をスケーリングする" };
const LiteGraph_Node_DefaultPadding = {
	"name": "新しいノードを常に縮小",
	"tooltip": "ノード作成時に可能な限り小さいサイズにリサイズします。無効にすると、新しく追加されたノードはウィジェットの値が表示されるように少し幅広くなります。"
};
const LiteGraph_Node_TooltipDelay = { "name": "ツールチップ遅延" };
...
//# sourceMappingURL=settings-CNcb_4nC.js.map